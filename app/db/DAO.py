...
class DAO:
			...
			try:
				...
				if len(rows) > 0:
					...
					for row in rows:
						drow = {"uuid": row[0], 
							"title": row[1],
							"artists": [{"uuid": row[2], "name": row[3], "relation": row[4]}],
	...
	def getReleasesByArtist(self, attr_name, attr_value, collection=None):
		...
		if conn:
			curs = conn.cursor()
			try:
				query = """SELECT DISTINCT rl.uuid, rl.title, rl.date, a.uuid, a.name, c.name
					FROM `release` rl, `artist` a, `collection` c
					WHERE a.%s='%s' 
	...
	def getReleasesByArtistRelation(self, attr_name, attr_value, collection=None):
		...
		if conn:
			curs = conn.cursor()
			try:
				query = """SELECT DISTINCT rl.uuid, rl.title, rl.date, a.uuid, a.name, rel.type, rel.attribute, c.name
				...
				if len(rows) > 0:
					...
					for row in rows:
						drow = {"uuid": row[0], 
							"title": row[1],
							"date": row[2],
							"artist": {"uuid": row[3], "name": row[4], "relation": {"type": row[5],
							"attribute": row[6]}},
	...
	def getReleasesByArtistInRecordings(self, attr_name, attr_value, collection=None):
		"""
		get the releases where the queried artist is the artist in some recordings, but he/she is not the main artist of the release in MusicBrainz
		"""
		str_collection = ""
		if collection:
			str_collection = "AND c.name = '%s'" % collection
		...
		if conn:
			...
			try:
				...
				if len(rows) > 0:
					drows = []
					for row in rows:
						release_artist = self.getReleaseInfo("uuid", row[0])["artist"]
						drow = {"uuid": row[0], 
							"title": row[1],
							"date": row[2],
							"artist": {"uuid": release_artist["uuid"], "name": release_artist["name"]},
							"collection": {"name": row[5]}
						}
						drows.append(drow)
					data = (True, drows, "")
	...
	def getReleasesByArtistInRecordingRelations(self, attr_name, attr_value, collection=None):
		"""
		get the releases where the queried artist has participated in some recordings, but he/she is not the main artist of the release in MusicBrainz
		"""
		str_collection = ""
		if collection:
			str_collection = "AND c.name = '%s'" % collection
				...
				if len(rows) > 0:
					...
					for row in rows:
						...
						drow = {"uuid": row[0], 
							...
							"date": row[2],
							"artist": {"uuid": release_artist["uuid"], "name": release_artist["name"]},
							"relation_artist": {"uuid": row[3], "name": row[4],
									    "relation": {"type": row[5], "attribute": row[6]}},
							"collection": {"name": row[7]}
						}
						drows.append(drow)
					data = (True, drows, "")
	...
	def getRecordings(self, uuid=None, collection=None, limit=None):
		...
		if conn:
			curs = conn.cursor()
			try:
				query = """SELECT DISTINCT r.uuid, r.title, r.length, a.uuid, a.name, t.id, t.tag, t.category, rl.uuid, rl.title, c.name
					FROM `recording` r, `artist` a, `annotation` ann, `tag` t, `release` rl, `collection` c
					%s r.collection_mbid = c.mbid
					AND a.uuid = r.artist_uuid
					AND rl.uuid = r.release_uuid
					%s
					AND ann.uuid_entity = r.uuid
					AND t.id = ann.tag_id 
				...
				if len(rows) > 0:
					...
					for row in rows:
						drow = { 
							...
							"length": int(row[2]),
							"artist": {"uuid": row[3], "name": row[4]},
							"tags": [{"id": row[5], "tag": row[6], "category": row[7].strip('1234567890')}],
							"release": {"uuid": row[8], "title": row[9]},
							"collection": {"name": row[10]}
						}
						#print drow["tags"]
						if not drows.has_key(drow["uuid"]):
							drows[drow["uuid"]] = drow
						else:
							drows[drow["uuid"]]["tags"].extend(drow["tags"])
					data = (True, drows.values(), "")
				else:
					data = (False, "empty", "")
			except MySQLdb.OperationalError, msg:
			...
			if len(rows) > 0:
				...
				for row in rows:
					...
					drow = {"uuid": row[0], 
						...
						"length": int(row[2]),
						"artist": {"uuid": row[3], "name": row[4]},
						"tag": {"name": row[5], "category": row[6].strip('1234567890')},
						"release": {"uuid": row[7], "title": row[8]},
						"collection": {"name": row[9]}
					}
					status, tags, msg = self.getTags(drow['uuid'])
					if status:
						drow['tags'] = [{'id': tag['id'], 'tag': tag['tag'], 'category': tag['category']} for tag in tags]
					drows.append(drow)
				data = (True, drows[:], "")
			else:
				data = (False, [], "")
	...
	def getRecordingsByArtist(self, attr_name, attr_value, collection=None):
		...
		if conn:
			curs = conn.cursor()
			try:
				query = """SELECT DISTINCT r.uuid, r.title, a.uuid, a.name, t.id, t.tag, t.category, rl.uuid, rl.title, c.name
					FROM `recording` r, `artist` a, `annotation` ann, `tag` t, `release` rl, `collection` c
					WHERE a.%s='%s'
					AND r.artist_uuid = a.uuid
					AND ann.uuid_entity = r.uuid
				...
				if len(rows) > 0:
					...
					for row in rows:
						drow = {"uuid": row[0], 
							...
							"tags": [{"id": row[4], "tag": row[5], "category": row[6].strip('1234567890')}],
							"release": {"uuid": row[7], "title": row[8]},
							"collection": {"name": row[9]}
						}
						print drow["tags"]
	...
	def getRecordingsByArtistRelation(self, attr_name, attr_value, collection=None):
		...
		if conn:
			curs = conn.cursor()
			try:
				query = """SELECT DISTINCT r.uuid, r.title, a.uuid, a.name, t.id, t.tag, t.category, rl.uuid, rl.title, rel.type, rel.attribute, c.name
					FROM `recording` r, `artist` a, `annotation` ann, `tag` t, `release` rl, `relation` rel, `collection` c
					WHERE a.%s='%s' 
					AND (rel.uuid_entity1 = a.uuid AND r.uuid = rel.uuid_entity2 OR 
						rel.uuid_entity1 = r.uuid AND a.uuid = rel.uuid_entity2)
					AND ann.uuid_entity = r.uuid
				...
				if len(rows) > 0:
					drows = {}
					for row in rows:
						if not release_artists.has_key(row[0]): # get release artist
							release_artists[row[0]] = self.getRecordingInfo("uuid", row[0])["artist"]
						drow = {"uuid": row[0], 
							"title": row[1],
							"artist": {"uuid": release_artists[row[0]]["uuid"],
								   "name": release_artists[row[0]]["name"]},
							"relation_artist": {"uuid": row[2], "name": row[3],
									    "relation": {"type": row[9],
							"attribute": row[10]}},
							"tags": [{"id": row[4], "tag": row[5], "category": row[6].strip('1234567890')}],
							"release": {"uuid": row[7], "title": row[8]},
							"collection": {"name": row[11]}
						}
						print drow["tags"]
						if not drows.has_key(drow["uuid"]):
							drows[drow["uuid"]] = drow
	...
	def getRecordingsByWork(self, attr_name, attr_value, collection=None):
		...
		if conn:
			curs = conn.cursor()
			try:
				query = """SELECT DISTINCT r.uuid, r.title, a.uuid, a.name, t.id, t.tag, t.category, rl.uuid, rl.title, w.uuid, w.title, c.name
					FROM `recording` r, `artist` a, `release` rl, `work` w, `annotation` ann, `tag` t, `relation` rel, `collection` c
					WHERE w.%s='%s'
					AND (rel.uuid_entity1 = w.uuid AND r.uuid = rel.uuid_entity2 OR 
						rel.uuid_entity1 = r.uuid AND w.uuid = rel.uuid_entity2) 
					AND r.release_uuid = rl.uuid
	...
	def __getRecordingsByInstrument(self, instrument, collection=None):
		...
		if conn:
			curs = conn.cursor()
			try:
				query = """SELECT DISTINCT r.uuid, r.title, r.length, a.uuid, a.name, t.id, t.tag, t.category, rl.uuid, rl.title, rel.attribute, c.name
					FROM `recording` r, `release` rl, `artist` a, `annotation` ann, `tag` t, `relation` rel, `collection` c
			...
			if len(rows) > 0:
				...
				for row in rows:
					drow = { 
						"id": int(row[0]),
						"tag": row[1],
						"category": row[2].strip('1234567890')
					}
					drows.append(drow)
				data = (True, drows[:], "")
			else:
